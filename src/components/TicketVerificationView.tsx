...
export const TicketVerificationView: React.FC<Props> = ({
  ...
  useEffect(() => {
    let isMounted = true;
    const loadQr = async () => {
      // Encode the fully qualified, active verification URL
  ...
  const handleSearch = (e: React.FormEvent) => {
    ...
    if (found) {
      ...
      setSearchError('No se encontró ningún ticket con ese código, DNI o número.');
    }
  };
  ...
  const handleShare = () => {
    const shareText = `🎟️ *CERTIFICADO DE TICKET OFICIAL*\n\n` +
    ...
    <div id="verification-screen-container" className="min-h-screen bg-[#F5F5F3] text-[#0F1115] py-8 px-4 flex flex-col items-center justify-between font-['Geist',sans-serif]">
      ...
      <div className="w-full max-w-md mt-6">
        <form onSubmit={handleSearch} className="relative">
          <input
            type="text"
            placeholder="Verificar otro código o número (ej: RF-82K7X91 o #1248)..."
            value={searchCode}
